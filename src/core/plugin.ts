...
function addInstanceMethods(schema: Schema): void {
  schema.methods.getHistory = async function(): Promise<ChronicleChunk[]> {
    ...
    return chunks as unknown as ChronicleChunk[];
  };

  schema.methods.createSnapshot = async function(name: string): Promise<ChronicleBranch> {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const doc = this as any;
...
function addStaticMethods(schema: Schema, options: ChroniclePluginOptions): void {
  schema.statics.findAsOf = async function(
    ...
    for (const doc of docs) {
      const asOfResult = await chronicleAsOfOp(ctx, doc._id, asOf);
  ...
  schema.pre('findOneAndUpdate', async function() {
    ...
    if (update && typeof update === 'object') {
      ...
      if (updateObj.$unset && typeof updateObj.$unset === 'object') {
        for (const key of Object.keys(updateObj.$unset as object)) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (this as any)._chronicleUpdatedDoc = doc;
